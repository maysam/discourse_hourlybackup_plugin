module DiscourseSubscriptionManager
  class AdminSubscriptionManagerController < Admin::AdminController
    requires_plugin 'discourse-subscription-manager'

    def add(rule)
      PluginStore.set("subscription-manager", 'rule', rule)
    end

    def index
      render_json_dump({
        posts: serialize_data(DiscourseSubscriptionManager.needs_review, PostSerializer),
        enabled: SiteSetting.subscription_manager_enabled?,
        stats: DiscourseSubscriptionManager.stats
      })
    end

    def confirm_spam
      post = Post.with_deleted.find(params[:post_id])
      DiscourseSubscriptionManager.move_to_state(post, 'confirmed_spam')
    ...
    def allow
      ...
      DiscourseSubscriptionManager.move_to_state(post, 'confirmed_ham')
    ...
    def dismiss
      ...
      DiscourseSubscriptionManager.move_to_state(post, 'dismissed')
