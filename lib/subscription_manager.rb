module SubscriptionManager
  ...
  def self.check_for_spam(to_check)
    ...
    spam_count = 0
    SubscriptionManager.with_client do |client|
      [to_check].flatten.each do |post|

        if post.user_deleted? || !post.topic
          SubscriptionManager.move_to_state(post, 'skipped')
          next
        end

        # If the post is spam, mark it for review and destroy it
        if client.comment_check(*SubscriptionManager.args_for_post(post))
          PostDestroyer.new(Discourse.system_user, post).destroy
          spam_count += 1
          SubscriptionManager.move_to_state(post, 'needs_review')
          ...
          SubscriptionManager.move_to_state(post, 'checked')
        end
      end
    end
  ...
  def self.move_to_state(post, state, opts=nil)
    ...
    msg = { akismet_review_count: SubscriptionManager.needs_review.count }
